...
//@todo: Est-ce que c'est necessaire d'inclure jquery ici?
...
function webExplorer_getManualActiveColorType()
{
	return window.parent.webExplorer_manualActiveColorType;
}

function webExplorer_getManualActiveColorLink()
{
	return window.parent.webExplorer_manualActiveColorLink;
}

function webExplorer_getManualActiveColorC()
{
...
function webExplorer_manualSetColor(nodeId)
{
	...
	for (var i = 0; i < nodeList.length; i++)
	{
		...
		{
			for (var j = 0; j < nodeList[i].nodeDomTreePath.length; j++)
			{
				if (nodeList[i].nodeDomTreeVisited[j] == true)
				{
					documentDom.each(function (index, element)
					{
						if (getElementPath($(this)) == nodeList[i].nodeDomTreePath[j])
						{
									...
									case 'background':
										switch (webExplorer_linkType)
										{
											...
											case 'el':
												...
												break;
											case 'elv':
												if (webExplorer_isExternalLink(nodeId, nodeList[i].nodeDomTreePath[j]) && webExplorer_hasBeenVisitedExternalLink(nodeId, nodeList[i].nodeDomTreePath[j]))
												{
													$(webExplorer_element).css('background-color', webExplorer_color);
